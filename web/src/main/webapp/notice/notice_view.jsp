...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
 <%
 //컨트롤러에서 1차 클래스 배열값을 JSP에서 받아서 출력하는 형식
  ArrayList<String> views = (ArrayList)request.getAttribute("notice_v");
 if(views == null){ //데이터가 없을 경우 스크립트 발동 후 리스트페이지로 이동
...
<a href="../notice_file/<%=views.get(5)%>" target="_blank"><%=views.get(5)%></a></div></span><br>
<%
  }
%>
<br><br>
...
function notice_info(p){
	...
	case 3:
		if(confirm("해당 게시물을 삭제 시 복구되지 않습니다.")){
			if(frm.npw.value==""){
				alert("게시물 삭제 시 패드워드를 입력하셔야 합니다.");
			}
			else{
				frm.submit();
			}
