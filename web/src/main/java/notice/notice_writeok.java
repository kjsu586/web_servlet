...
@MultipartConfig(
	...
	maxFileSize = 1024 * 1024 * 50, //최대용량은 파일사이즈의 10배 정도로 잡는다
	maxRequestSize = 1024 * 1024 * 500 //파일이 여러개일 경우만 사용
)
public class notice_writeok extends HttpServlet {
    ...
    //원래는 PrintWriter 이거 말고 위 4개는 Model에서 불러오는게 맞다. 컨트롤러에서는 PrintWriter 이것만!
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(filesize == 0) { //첨부파일 없을 경우
				...
				//return 메소드가 private이기 때문에 전역변수의 값을 직접 컨트롤러에 로드함
				String msg = nt.msg;
			...
			this.pw.write("<scirpt>"
					+ "alert('데이터베이스 문제로 인하여 저장이 되지 않았습니다.');"
					+ "history.go(-1);"
					+ "</script>");
