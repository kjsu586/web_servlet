...
public class pollok extends HttpServlet {
	...
	//RequestDispatcher -> Controller -> view (jsp) 결과물 처리
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");	//Front-end 한글 깨짐
		//response.setCharacterEncoding("utf-8");
		//response.setContentType("text/html");
		response.setContentType("text/html;charset=utf-8");
		
		//동일한 name -> radio : 여러개의 오브젝트 중에 한가지만 선택
		//getParameter : get,post (name전달됨 값 동일함)
		//request.getParameter : 무조건 Front-end 값이 이관돼야 함
